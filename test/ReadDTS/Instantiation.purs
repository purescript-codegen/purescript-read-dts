...
-- import TypeScript (CompilerHost, createCompilerHost, moduleKind, scriptTarget) as TypeScript
-- 
-- lines :: forall f. Foldable f => f String -> String
-- lines = intercalate "\n"
-- 
-- suite :: TypeScript.CompilerHost → TestSuite
-- suite compilerHost = Test.suite "ReadDTS.Instantiation" $ do
...
--         ]
--     liftEffect (AST.build ReadDTS.defaults file compilerHost) >>= case _ of
...
--           }
--         ]
--     liftEffect (AST.build compilerOptions file compilerHost) >>= map (Array.filter (eq (Just "X") <<< typeName)) >>> case _ of
--       Right [tc] → do
...
--       compilerOptions = ReadDTS.defaults
-- 
--     liftEffect (AST.build compilerOptions file compilerHost) >>= case _ of
--       Right [tc] → runExcept (instantiate tc []) # case _ of
--         Right (Mu.In obj@(Object n o)) → do
