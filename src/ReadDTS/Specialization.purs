module ReadDTS.Specialization where
...
--   { params ∷ Array (AST.TypeNode ref)
--   , constructor ∷ AST.TypeDeclaration ref
--   }
...
--     = foldMap (foldMap f) d.params
--     <> foldMap f d.typeConstructor
--   foldr f t = foldrDefault f t
--   foldl f t = foldlDefault f t
-- 
-- instance traversableApplication ∷ Traversable Application where
--   sequence (Application d@{ typeConstructor, params }) = map Application
--     $ (\tc ta → d { typeConstructor = tc, params = ta })
--     <$> (sequence typeConstructor)
--     <*> (sequence <<< map sequence) params
--   traverse = traverseDefault
-- 
-- type Type = Mu TypeF
-- 
-- type Specialize = Map String Type → Except String Type
-- 
-- -- specializeApplication ∷ Algebra TypeNode Specialize
-- -- specializeApplication (Application {  = \ctx → case t of
-- --   AST.Application { params: params, constructor: toTypeDeclaration } -> do
-- --     typeDeclaration ← toTypeDeclaration ctx
-- --     case typeDeclaration of
-- --       AST.Interface { fullyQualifiedName, properties, typeParameters } → do
-- --         params' ← traverse (flip specialize ctx) params
-- --         let
-- --           ctx' = Map.fromFoldable (Array.zip (map _.name typeParameters) params')
-- --         roll <$> Object (fqnToString fullyQualifiedName) <<< Map.fromFoldable <$> for properties \{ name, type: t, optional } →
-- --           (Tuple.Tuple name <<< { type: _, optional }) <$> specialize t ctx'
-- --       AST.Module { declarations, fullyQualifiedName } → do
-- --         roll <<< Module <<< { fullyQualifiedName: fqnToString fullyQualifiedName, types: _ } <$> for declarations \tc → do
-- --           specialize (AST.Application { params: [], typeDeclaration: tc }) ctx
-- --       AST.TypeAlias { type: t, typeParameters } → do
-- --         params' ← traverse (flip specialize ctx) params
-- --         let
-- --           ctx' = Map.fromFoldable (Array.zip (map _.name typeParameters) params')
-- --         specialize t ctx'
-- --       AST.Class c → pure $ roll $ Unknown ("Class specialization not implemented yet")
-- --       AST.UnknownTypeDeclaration r → pure $ roll $ Unknown
-- --         ("Unknown type constructor: " <> show r.fullyQualifiedName <> ". params: " <> unsafeStringify params)
-- -- 
...
-- --       ((Tuple.Tuple name <<< { type: _, optional }) <$> specialize t ctx)
-- --   AST.Array t → roll <$> Array <$> specialize t ctx
-- --   AST.Boolean → pure $ roll Boolean
-- --   AST.BooleanLiteral b → pure $ roll $ BooleanLiteral b
-- --   AST.Function fd → do
-- --     parameters ← traverse (\r@{ name } → { name, type: _ } <$> specialize r.type ctx) fd.parameters
-- --     returnType ← specialize fd.returnType ctx
-- --     pure $ roll $ Function { parameters, returnType }
-- --   AST.Intersection ts →
-- --     traverse (flip specialize ctx) ts >>= Array.reverse >>> Array.uncons >>> case _ of
...
-- --       Just t, _ → pure t
-- --       Nothing, Just d → specialize d ctx
-- --       _, _ → pure $ roll $ Unknown name
-- --       -- _, _ → throwError ("Variable not defined: '" <> name <> "'. Current context: " <> show (Map.keys ctx) <> ".")
-- --   AST.Tuple ts → roll <$> Tuple <$> traverse (flip specialize ctx) ts
-- --   AST.Undefined → pure $ roll Undefined
-- --   AST.Union ts → roll <$> Union <$> traverse (flip specialize ctx) ts
...
-- -- 
-- -- -- -- instantiate
-- -- -- --    ∷ TypeDeclaration (Application')
-- -- -- --    → Array Type
-- -- -- --    → Except String Type
-- -- -- -- instantiate tc args = specialize application Map.empty
-- -- -- --   where
-- -- -- --   application = Application
-- -- -- --     { params: map (AST.ApplicationWithRef <<< const <<< pure) args
-- -- -- --     , typeDeclaration: map (cata specialize) tc
-- -- -- --     }
-- -- -- -- 
-- -- 
-- -- -- -- 
